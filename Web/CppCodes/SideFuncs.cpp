...
void changeErrorText(string errorText) {
  ofstream errorJS;
  errorJS.open("static/jsFiles/error.js");
  if (errorText == "") errorJS << " ";
  else errorJS << "alert(\'" << errorText << "\');";
  errorJS.close();
}
void changeContentJS(const string &text) {
  ofstream contentJS;
  contentJS.open("static/jsFiles/content.js");
  contentJS << "document.getElementById(\"div_\").innerHTML='" << text << "';";
  contentJS.close();
}